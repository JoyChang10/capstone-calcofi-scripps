...
data = pd.read_csv(
    r"C:\ANISH-OLDLENOVO\Anish_School\CalCofi_capstone\capstone-calcofi-scripps\preliminaryanalysis\preliminarydata.csv"
)
...
if selected_species is None:
    data["plot_value"] = data[species_cols].fillna(0).sum(axis=1)
    filename_suffix = "all_species"
else:
    data["plot_value"] = data[selected_species].fillna(0)
...
sizes = data["plot_value"] / data["plot_value"].max()
...
ax.set_title(f"Spatial Distribution ({filename_suffix})")
ax.set_axis_off()

plt.tight_layout()

output_dir = Path("preliminaryanalysis/Anish/visualizations")
output_dir.mkdir(parents=True, exist_ok=True)

output_path = output_dir / f"spatial_distribution_{filename_suffix}.png"
...
sizes = gdf["plot_value"] / gdf["plot_value"].max()
sizes = sizes * 200
...
ax2.set_title(f"Spatial Distribution of Dominant Species ({filename_suffix})")
ax2.set_axis_off()

plt.tight_layout()

output_path2 = output_dir / f"dominant_species_map_{filename_suffix}.png"
